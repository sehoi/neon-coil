 ...
 */
export const TILES = { min: 30, max: 300, cruise: 210, peak: 10 };
 ...
 *
 * `price` 는 **30장 판(레벨 1) 기준값**이다. 실제 값은 `itemPrice()` 가 판 크기에
 * 맞춰 올린다 — 안 그러면 판이 커질수록 골드가 값을 압도적으로 앞지른다
 * (실측: 레벨 10 클리어 한 번이 791골드인데 최고가 아이템이 220골드라, 값이
 * 고정이면 그 한 번으로 최고가 아이템을 서너 개씩 살 수 있었다).
 */
 ...
 * 지금 판 크기에 맞춘 아이템 값.
 *
 * 골드는 판 크기(정확히는 매치 수)에 거의 그대로 비례해 는다 — 점수의 대부분이
 * "3장 맞출 때마다 60점"에서 나오기 때문이다. 그래서 값도 같은 비율로 올린다:
 * 30장(레벨 1) 기준값 그대로, 300장(레벨 10)이면 10배. 10원 단위로 반올림한다.
 *
 * 300장 기준으로 보면 되돌리기 600 · 빼내기 1500 · 뒤집기 1100 · 섞기 2200 —
 * 그 판 클리어 한 번(약 791골드)으로는 되돌리기 하나도 빠듯하다. 예전에는
 * 값이 고정이라 그 한 번으로 최고가 아이템을 서너 개씩 살 수 있었다.
 */
export function itemPrice(name, tiles) {
  const base = ECONOMY.price[name] || 0;
  const scale = Math.max(1, tiles / TILES.min);
